...
localStatus= f"{localhost}/status"
	...
	def GET(self):
		...
		if(input):
			...
			if(input.status=="refresh"):
				...
				response = requests.get(localStatus).text
				estado = json.loads(str(response))
				...
				response = requests.get(localStatus).text
				estado = json.loads(str(response))
				estado.update({"LED_Light": not estado['LED_Light']})
				print("Updated status")
				print (estado)
				requests.post(localStatus,params= {"LED_Light": estado["LED_Light"]})
				#print(status)
		...
		try:
			...
			estado = {"LED": "No se pudo abrir el archivo"}
		#return status
		return self.render.status(estado)
